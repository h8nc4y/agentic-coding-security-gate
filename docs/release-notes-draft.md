...
- Scanner coverage now includes additional cloud, Slack, Stripe, bearer-token, PEM, path-like private marker, Anthropic, JWT-shaped, GitHub classic token, GitLab, Hugging Face, Slack incoming webhook, SendGrid, and npm auth-token patterns while keeping findings redacted.
