...
4. **実装**: 変更は最小限・首尾一貫に。フロントの**ビジュアルデザイン**が必要になったら、§12 の内部デザイン判断メモを使い、設計・実装・検証を一貫して担当する。
...
- **外部レビューは必要時のみ**: 大きめのリファクタ、設計判断が割れる、セキュリティ上重大、自分の自信が持てない時。§13 の依頼ブロックは任意の補助として使えるが、レビュー待ちを通常の停止条件にはしない。

---

## §12. フロントのビジュアルデザイン（内部判断メモ雛形同梱）

- 現状このリポジトリに**フロントエンド/UIは無い**（PowerShell + Markdown のみ）。
- 2026-07-11 に開発領域の固定分掌を廃止した。将来、可視化ダッシュボード・バッジ・レポートUI・サイト等で **配色 / 書体 / レイアウト** が必要になったら、Codex が方針決定・実装・ブラウザ検証まで担当する。
- 以下のメモで既存資産、アクセシビリティ、public-safe 境界、採用理由を記録する。外部のデザイン相談は任意であり、着手・実装・マージの必須ゲートにはしない。
...
セルフレビューで足りない時だけ任意で使う。依頼テキストも公開安全（合成・redact）に保つ（§5）。
