...
- Scanner regression coverage for Anthropic key prefixes, compact JWT-shaped token values, GitHub classic token prefixes, and npm registry `_authToken` assignment patterns.
...
- Scanner coverage now includes additional cloud, Slack, Stripe, bearer-token, PEM, path-like private marker, Anthropic, JWT-shaped, GitHub classic token, and npm auth-token patterns while keeping findings redacted.
