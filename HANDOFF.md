...
- T-001〜T-033、Pester 0-tests false green（PR #37）、secret-assignment の prefix / placeholder 非対称、dotenv filename coverage は現行 tree で完了。
...
## 最終検証結果（2026/08/02、T-033 Windows batch text coverage）

| 種別 | 結果 |
| --- | --- |
| TDD RED | 既存60 cases + boundary self-test は pass。追加 batch case のみ exit 0 で期待 exit 1 に失敗 |
| PowerShell 7 | 最終 scanner regression 61 cases + boundary self-test pass |
| Windows PowerShell 5.1 | 最終 scanner regression 61 cases + boundary self-test pass（補助証跡。公式 support は `pwsh` 7+） |
...
| Semgrep | direct base/current wrapperは実行policyにrejectされ未確認。統合判断はcommit時のglobal guard実測を用いる |
